...
import { ShoppingBag, MessageCircle, Heart, User, Package, LogOut, Search, MapPin, CreditCard, X, Plus, Trash2, Home, ShoppingCart, Check, Clock, CheckCircle, Truck, ChefHat } from "lucide-react";
import { db } from "@/lib/firebase";
import { collection, query, where, getDocs, doc, updateDoc, getDoc, arrayRemove, onSnapshot } from "firebase/firestore";
...
export default function CustomerDashboard() {
    ...
    useEffect(() => {
        ...
        if (chatId && activeTab === "messages") {
            // We need to fetch the chat details or just set the ID and let ChatWindow handle it?
            // ChatWindow needs more than just ID usually (recipientName), but let's see.
            // ChatList fetches chats. If we pass selectedChat, ChatWindow opens.
            // We might need to wait for ChatList to load to select it, or fetch it individually.
            // For simplicity, let's just set the tab for now. 
            // If we want to auto-open, we might need to fetch the chat doc here.
    ...
    useEffect(() => {
        if (customerId) {
            ...
            const unsubscribe = onSnapshot(q, (snapshot) => {
                ...
                setOrders(ords.sort((a, b) => b.timestamp?.seconds - a.timestamp?.seconds));
                setLoading(false);
            });
            return () => unsubscribe();
        }
    ...
    const getStatusIcon = (status) => {
        ...
        switch (s) {
            ...
            case "accepted": return <Check className={iconClass} />;
    ...
    return (
        <div className="min-h-screen bg-gray-50 pb-24">
            ...
            <div className="flex overflow-x-auto bg-white border-b border-gray-200 px-4 scrollbar-hide">
                {["orders", "messages", "wishlist", "profile"].map((tab) => (
                    <button
                        key={tab}
            ...
            <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 flex justify-around p-3 z-20">
                ...
                <button
                    ...
                    className={`flex flex-col items-center gap-1 ${activeTab === "orders" ? "text-pink-600" : "text-gray-400 hover:text-pink-600"}`}
                >
                    <Package size={24} />
                            ...
                            <form onSubmit={handleAddAddress} className="space-y-4">
                                ...
                                <div>
                                    ...
                                    <button
                                        ...
                                        className="mt-2 w-full py-2 border border-pink-600 text-pink-600 rounded-xl font-medium hover:bg-pink-50 transition-colors disabled:opacity-50"
                                    >
                                        {loadingLocation ? "Getting location..." : "📍 Use Current Location"}
                                    </button>
                                    {newAddress.latitude && newAddress.longitude && (
                                        <p className="mt-2 text-xs text-gray-500">
                                            Location saved: {newAddress.latitude.toFixed(6)}, {newAddress.longitude.toFixed(6)}
                                        </p>
                                    )}
                                </div>
                                <button
